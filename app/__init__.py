...
from app.tools import open_dialog as dialog
from app.services import diretory_repository as repo_directory
from app.services import file_repository as repo_file
...
def GUI():
    ...
    tk.Button(origen_frame, text="+", command=lambda:dialog.seleccionar_carpeta_origen(entrada_origen)).pack(side=tk.LEFT)
    ...
    tk.Button(destino_frame, text="+", command=lambda:dialog.seleccionar_carpeta_destino(entrada_destino)).pack(side=tk.LEFT)

    frame_botones = tk.Frame(tab_directory)
    frame_botones.pack(pady=10)
    tk.Button(frame_botones, text="PDF to Word", command=lambda:repo_directory.convertir_pdf_a_word(entrada_origen,entrada_destino,text_area)).pack(side=tk.LEFT, padx=10)
    tk.Button(frame_botones, text="Word to PDF", command=lambda:repo_directory.convertir_docx_a_pdf(entrada_origen,entrada_destino,text_area)).pack(side=tk.LEFT, padx=10)
    ...
    tk.Button(origen_frame, text="+", command=lambda: dialog.seleccionar_archivo(entrada_file)).pack(side=tk.LEFT)

    # Frame File

    frame_botones_file = tk.Frame(tab_file)
    frame_botones_file.pack(pady=10)
    tk.Button(frame_botones_file, text="PDF to Word", command=lambda: repo_file.convertir_pdf_to_word(entrada_file, text_area)).pack(side=tk.LEFT, padx=10)
    tk.Button(frame_botones_file, text="Word to PDF", command=lambda: repo_file.convertir_docx_to_pdf(entrada_file, text_area)).pack(side=tk.LEFT, padx=10)
